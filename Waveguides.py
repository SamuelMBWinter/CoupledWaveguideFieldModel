...
def DifferenceFucntion(xi, V, omega, s1, s2, p1, p2, r2, kappa, C):
    ...
    dFUf = (0-1j) * r2*(2*np.pi*omega)**2 * FUf + (0+1j) * fft(Us*np.conjugate(Uf)* np.exp((0+1j)*kappa *xi), norm="forward")
    
    dFUs = (0-1j)*(s1 * 2 * np.pi*omega + s2 * (2*np.pi*omega)**2) * FUs + (0+1j) * fft(0.5*Uf**2* np.exp((0-1j)*kappa *xi), norm="forward") + (0-1j) * C * FUp

    dFUp = (0-1j)*(p1 * 2 * np.pi * omega + p2 * (2*np.pi*omega)**2) * FUp + (0-1j) * C * FUs
...
class WaveguidePairModel:
    ...
    def RunSimulation(self, t_0, length, seperation, n=512, t_range=[-150, 150], method='DOP853'):
        """
        length : float
